...
HP-Agent 是一个基于 FastAPI、Vue 3 和 LLM 的英文阅读辅助工具。用户输入英文段落后，系统会按照所选阅读场景和阅读水平自动完成生词标注、中文释义插入和流式渲染；同时支持生词本、历史记录、点击查词和已掌握词过滤等功能，适合用于英文原著阅读、语言学习、技术文档和专业术语理解。

当前项目提供通用阅读、小说文学、哈利波特、技术文档、学术论文、新闻商业等场景预设。《哈利·波特》仍作为一个可选示例场景保留，用来展示系统如何处理魔法术语、人物称谓、小说叙事语境和具有作品背景的词汇，但项目默认定位已经扩展为更加通用的英文阅读助手。

该项目更多是为了满足个人学习和阅读需求而开发，你可以根据自己的需要修改语言种类或功能逻辑，将其适配到其他语言学习场景中。

## 项目定位

本项目的核心定位是“英文原著阅读助手”，重点解决用户在阅读英文长文本时遇到的三类问题：生词影响理解、长句和上下文翻译成本较高、已学词汇缺少持续管理。

《哈利·波特》在本项目中主要作为一个可选阅读场景，用来展示系统如何处理英文小说中的人物、地点、魔法术语和叙事表达。例如，系统可以针对 spell、wand、Muggle、Hogwarts 等具有作品语境的词汇给出更符合小说背景的中文解释，而不是只提供普通词典释义。

由于不同阅读材料的难点不同，系统现在将“阅读场景”和“阅读水平”拆开处理。例如，阅读英文小说时可以强调人物关系、习语和叙事语气；阅读计算机技术文档时可以强调 API、命令、参数和专业术语；阅读学术论文时可以强调领域概念、方法名称和句子逻辑。这样可以让同一套阅读工具更好地适配不同类型的英文文本。
...
- 提供通用阅读、小说文学、哈利波特、技术文档、学术论文、新闻商业等阅读场景预设。
...
│       ├── agent1.py                # 批注服务：调用 LLM、解析 JSON、返回标注结果
│       ├── agent2.py                # 点击查词与整句翻译服务
│       ├── prompt_profiles.py       # 阅读场景预设、分级规则与 prompt 构造
...
│       ├── views/
│       │   ├── ReadingPage.vue      # 阅读页面、场景预设与水平选择器
...
当前标注提示词已经从 `agent1.py` 中抽离到 `backend/src/hp_agent/prompt_profiles.py`。系统使用一份通用基础规则保证输出格式和上下文准确性，再根据前端选择的阅读场景和阅读水平动态组合用户提示词。

阅读场景用于决定“这是什么类型的英文材料”，阅读水平用于决定“标注密度和解释深度”。当前内置场景如下：

| 场景值 | 前端显示 | 提示词优化重点 |
| ------ | -------- | -------------- |
| `general` | 通用阅读 | 上下文含义、短语动词、习语、文化表达和常见词的特殊语义。 |
| `fiction` | 小说文学 | 人物动作、叙事语气、对白口吻、文学性表达、隐喻和场景描写。 |
| `harry_potter` | 哈利波特 | 魔法术语、英式口语、学校称谓、咒语、魔法物品、货币和作品语境词。 |
| `technical` | 技术文档 | API 名称、命令行参数、配置项、框架概念、错误信息和工程术语。 |
| `academic` | 学术论文 | 研究方法、理论概念、抽象名词、论证连接词、指标和领域术语。 |
| `news_business` | 新闻商业 | 机构名称、政策术语、金融经济表达、行业术语、缩写和事件背景。 |

阅读水平继续使用 `beginner`、`intermediate`、`advanced` 三档，用于控制标注密度。前端默认使用 `general + intermediate`，用户可以在阅读页面切换场景预设。哈利波特不再是全局默认提示词，而是作为 `harry_potter` 场景保留。

这种设计的优点是：标注服务主体逻辑不需要频繁修改，只需要在 `prompt_profiles.py` 中维护场景预设，就可以持续扩展不同类型的英文阅读材料。
...
```text
用户输入英文文本并选择阅读场景与阅读水平
  ↓
POST /api/create-process-task，提交 text、profile、level 并创建处理任务
  ↓
EventSource 连接 /api/process-stream?task_id=...
  ↓
从 SQLite 查询已掌握词汇，并与阅读场景、阅读水平一起写入 LLM prompt
  ↓
DocumentProcessor 按段落和词数切分文本
  ↓
使用 asyncio.Semaphore(3) 并行请求 LLM
  ↓
AnnotatorService 根据阅读场景和阅读水平生成批注结果
  ↓
...
本项目主要面向英文阅读学习与个人工具开发场景，用于辅助理解英文文本、生词和上下文含义。《哈利·波特》只是当前版本保留的一个可选场景预设，实际使用时可以根据目标材料选择通用阅读、小说文学、技术文档、学术论文或新闻商业等模式。使用前请确保已正确配置 LLM API Key，并注意保护个人密钥安全。
